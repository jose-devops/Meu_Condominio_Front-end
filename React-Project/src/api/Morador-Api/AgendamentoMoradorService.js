...
export const cadastrarAgendamentoMorador = async (dadosAgendamentoProprietario) => {
  const token = localStorage.getItem('token'); 
...
export const listarAgendamentosMorador = async () => {
  const token = localStorage.getItem("token");
  const config = {
    headers: {
      Authorization: `Bearer ${token}`
...
export const editarAgendamentoMorador= (dados) => {
  const token = localStorage.getItem('token');
...
export const deletarAgendamentoMorador = (id, token) => {
